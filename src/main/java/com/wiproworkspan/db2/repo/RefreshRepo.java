...
@Repository
public interface RefreshRepo extends JpaRepository<MaintainJobs, Integer>{

	@Query(value="SELECT DISTINCT xjs.schedule_id,xjs.schedule_name,hold_flag, IFNULL(xjs.next_scheduled_run_date,CONCAT(DATE(xjs.schedule_start_date), ' ', TIME(resubmit_time))) AS next_scheduled_run_date,xj.reference_group_id FROM xxws_job_schedules xjs, xxws_jobs xj WHERE xjs.job_id = xj.job_id and xjs.schedule_id= :sch_id order by xjs.schedule_name", nativeQuery = true)
	List<Object[]> findAllMaintainJobs(@Param("sch_id") int sch_id);
